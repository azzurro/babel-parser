...
class StatementParser extends _expression.default {
  ...
  canHaveLeadingDecorator() {
    if (this.match(_types2.types._const)) {
      const oldState = this.state;
      let found = false;
      this.state = oldState.clone(true);

      while (!(this.match(_types2.types.eof) || this.match(_types2.types.semi))) {
        this.nextToken();

        if (this.match(_types2.types._function) || this.match(_types2.types.arrow)) {
          found = true;
          break;
        }
      }
    ...
    return this.match(_types2.types._class) || this.match(_types2.types._function);
  }
  ...
  parseDecorator() {
    ...
    this.state.decoratorStack[this.state.decoratorStack.length - 1] = currentContextDecorators;
    return result;
  }
