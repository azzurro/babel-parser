...
class ExpressionParser extends LValParser {
  ...
  parseSubscript(base, startPos, startLoc, noCalls, state) {
    ...
    if (optional && !this.match(types.parenL) && !this.match(types.backQuote) || computed || this.eat(types.dot)) {
      const node = this.startNodeAt(startPos, startLoc);
      node.object = base;
      node.property = computed ? this.parseExpression() : optional ? this.parseIdentifier(true) : this.parseMaybePrivateName();
  ...
  parsePropertyName(prop) {
    if (this.eat(types.bracketL)) {
      ...
      prop.key = this.match(types.num) || this.match(types.string) || this.match(types.bigint) ? this.parseExprAtom() : this.parseMaybePrivateName();
...
class StatementParser extends ExpressionParser {
  ...
  canHaveLeadingDecorator() {
    if (this.match(types._const)) {
      const oldState = this.state;
      let found = false;
      this.state = old.clone(true);

      while (!(this.match(types.eof) || this.match(types.semi))) {
        this.nextToken();

        if (this.match(types._function) || this.match(types.arrow)) {
          found = true;
          break;
        }
      }
    ...
    return this.match(types._class) || this.match(types._function);
  }
